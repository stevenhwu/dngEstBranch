...
callDngLikeli<- function(params){
  ...
  lib<- invLogit(params[2])

  dngOtherArgs <- paste("-t 20 --mu-somatic ", som, " --mu-library", lib)
...
names(mu)<- c("somatic", "library")
log_hidden <- baseLikeli/result$value

output<- list(mu_somatic=mu["somatic"], mu_library=mu["library"], log_hidden=log_hidden, baseLikeli=baseLikeli, result=result)
print(output)

result_id<- paste0(paste0(name,collapse="_"), "_", pid)
outfile <- paste0("optimDngSomLib_", result_id, ".RData" )
